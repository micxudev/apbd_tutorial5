...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps
            ...
            {
                EName = emp.EName,
                Total = emp.Sal + (emp.Comm == null ? 0 : emp.Comm)
            }); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            ).SelectMany(grp => grades
                .Where(grade => grp.Sal >= grade.Losal && grp.Sal <= grade.Hisal)
                .Select(grade => new { grp.EName, grp.DName, grade.Grade })
            ); 
